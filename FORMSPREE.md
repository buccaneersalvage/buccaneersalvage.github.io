...
| **eBay listing services** (`services.html` / `square.link`) | **Done via Square API 2026-09-02** (10 services links + Partner `$59` `2Fj74DTt`, Maintain `$29` `nj6AI2f0` (allotment title fixed)). Re-run if links are recreated. Manual path was: Square Dashboard → Payment Link → After payment redirect → `https://buccaneersalvage.github.io/contact.html?source=square-services&topic=Listing%20services` so buyers can send photos / PNs / store URL via Formspree `mlggwlyo`. |
